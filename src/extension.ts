...
interface Minmax {
  //Class for storing argument name, min & max values
  argument: string;
  min: number;
  max: number;
}
...
export function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand("nanofuzz.Fuzz", async () => {
      //Adding the Fuzz command through the extension.
      const editor = vscode.window.activeTextEditor; //The page you are writing code on
      if (!editor) {
        //Catch for nulls
        return;
      }
      let funcpos = editor.document.getWordRangeAtPosition(
        editor.selection.active
      ); //Get the the line number and character number for clicked cursor position
      ...
      terminal.sendText(
        `ts-node -e \'` +
          buildSequence(functionPath, funcName, fuzzerPath) +
          `\'`
      ); //Prints out values to terminal automatically, using the buildSequence helper function
    })
  );
  context.subscriptions.push(
    vscode.commands.registerCommand("nanofuzz.AskPrompt", () => {})
  );
}
...
var buildSequence = (
  ...
  const importFunction =
    ...
    functionName +
    ` } from "` +
    functionPath.substring(0, functionPath.length - 3) +
    `";`; // Get the function file imported into terminal.
...
var argSynthesizer = (func: Function) => {
  //Dummy function for returning the arguments;
