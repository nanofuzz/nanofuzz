...
export class CompositeInputGenerator extends AbstractInputGenerator {
  ...
  public next(): InputAndSource {
    ...
    // efficient subgen
    // !!!!!!!! Move to a single probability space
    let gen: AbstractInputGenerator;
    const rand = Math.floor(this._prng() * this._explorationP);
    const G = this._subgens.filter((g) => g.isAvailable()).length;
    if (rand === 0) {
    ...
    this._lastInput = gen.next(); // TODO Track execution cost here !!!!!!! also handle exceptions
    return JSON5.parse(JSON5.stringify(this._lastInput));
  }
